...
function generatePolicyContent(type) {
    ...
    return `
        <div style="max-width: 800px; margin: 0 auto; background: white; padding: 40px; border-radius: 12px; box-shadow: 0 4px 6px rgba(0,0,0,0.1);">
            <button onclick="goBackToHome()" style="margin-bottom: 20px; background: #0066CC; color: white; border: none; padding: 10px 20px; border-radius: 6px; cursor: pointer;">← Back to Home</button>
...
function createLaptopCard(laptop) {
    ...
    const imageHtml = laptop.images && laptop.images.length > 0 
        ? `<img src="${laptop.images[0]}" alt="${laptop.brand} ${laptop.model}" onerror="this.style.display='none'; this.nextElementSibling.style.display='flex';">
           <div style="display:none; align-items:center; justify-content:center; font-size:4rem; color:#6C757D;">💻</div>`
        : `<div style="display:flex; align-items:center; justify-content:center; font-size:4rem; color:#6C757D;">💻</div>`;
    // Stock status
    ...
    card.innerHTML = `
        ...
        <div class="laptop-details">
            ...
            <div class="laptop-pricing">
                <div class="original-price">₹${laptop.original_price.toLocaleString()}</div>
                <div class="current-price">₹${laptop.sell_price.toLocaleString()}</div>
...
// Enhanced viewLaptopDetails function to open modal with laptop details
